...
for($i=0;$i < $t; $i++)
	...
	if($patientid != $_COOKIE[pi] && $_POST['test'] != $_COOKIE[ct]) {
		...
		if($cnnn==0)
			{
			$links->query("UPDATE test SET pathologist='$_POST[pt]',medtech='$_POST[mt]', result='$result',daterelease='$daterelease' , note = '$_POST[note]', specimen='$specimen_data' where testid='$id'") or die($links->error); //save result
			}else{
           $links->query("UPDATE test SET pathologist='$_POST[pt]', medtech='$_POST[mt]',result='$result',daterelease='$daterelease', note = '$_POST[note]', specimen='$specimen_data'where testid='$id'") or die($links->error); //save result
			 }
	}
...
if(($tot=="Special Tests") || ($tot=="Semen Analysis") || ($tot=="Differential Count") || ($tot=="PROTIME") || ($tot=="APTT") || ($tot=="Dengue Duo") || ($tot=="OGTT") || ($tot=="Electrolytes") || ($tot=="Salmonella Typhi IgG and IgM") || ($tot=="T3, T4 and TSH") || ($tot=="FT3 and FT4")) {
...
$pdf->Line(179,$y,190,$y);


//$pdf->Cell(150,6,"",0,0,'L',true);
//$y = $pdf->GetY() + 5;

//$pdf->Cell(550,6,"Date : " .date("Y-m-d"),0,0,'L',true);
	...
	if($testarr[$i]=="Laboratory") {
		...
		/******************************************end of marker*******************************/
			...
			elseif($tot=="Salmonella Typhi IgG and IgM")
			{
				...
				$pdf->Cell(120,7,"Serum:    ".$specimen[2],'0',0,'L',false);
				$y = $pdf->GetY() + 6;
                $pdf->Line(25,$y,100,$y);
				$pdf->Ln();
			    $pdf->Cell(120,7,"Examination Desired : Salmonella Typhi IgG and IgM",'0',0,'L',false);
				$y = $pdf->GetY() + 6;
                $pdf->Line(50,$y,120,$y);
				$pdf->Ln(8);
				...
				$pdf->Cell(90,6,"IgG Antibody for Salmonella Typhi:...............",'0',0,'L',false);
				$pdf->Cell(40,6,$specimen[0],B,0,'L',false);
				$pdf->Ln(8);
				$pdf->SetX(40);
				$pdf->Cell(90,6,"IgM Antibody for Salmonella Typhi:...............",'0',0,'L',true);
				$pdf->Cell(40,6,$specimen[1],B,0,'L',false);
				$pdf->Ln(10);
	...
	elseif($testarr[$i]=="Ultrasound")
		...
		// echo $rows[id] .">" .$results[$rows[id]];
			if($results[$rows[labid]] != "")
				{
				if($rows[typeoftest] != NA || $rows[typeoftest] != N/A || $rows[typeoftest] != na) {
